...
import { Slider } from '@/components/ui/slider';
import { Badge } from '@/components/ui/badge';
import  VideoStream  from '../../components/3d/VideoStream';
...
function Cube(props: any) {
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      meshRef.current.rotation.x += delta * (active ? 1.5 : 1);
      meshRef.current.rotation.y += delta * (active ? 1.5 : 1);
    }
  });
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial color={hovered ? '#B3A1C6' : '#6A4A8E'} />
    </mesh>
  );
}
        ...
        <div className="relative aspect-video bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden">
          <div ref={mountRef} className="w-full h-full">
            {isLoading ? (
              ...
              //   <Canvas camera={{ position: [2, 2, 2], fov: 75 }} shadows>
